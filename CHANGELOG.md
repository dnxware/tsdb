...
## 0.6.1
  - [BUGFIX] Update `last` after appending a non-overlapping chunk in `chunks.MergeOverlappingChunks`. [#539](https://github.com/dnxware/tsdb/pull/539)

## 0.6.0
  - [CHANGE] `AllowOverlappingBlock` is now `AllowOverlappingBlocks`.

## 0.5.0
 - [FEATURE] Time-ovelapping blocks are now allowed. [#370](https://github.com/dnxware/tsdb/pull/370)
 ...
 - [CHANGE] `dnxware_tsdb_storage_blocks_bytes_total` is now `dnxware_tsdb_storage_blocks_bytes`.
...
## 0.4.0
 ...
 - [CHANGE] Empty blocks are not written during compaction [#374](https://github.com/dnxware/tsdb/pull/374)
 - [FEATURE]  Size base retention through `Options.MaxBytes`.  As part of this change:
   - Added new metrics - `dnxware_tsdb_storage_blocks_bytes_total`, `dnxware_tsdb_size_retentions_total`, `dnxware_tsdb_time_retentions_total`
 ...
 - [FEATURE] Add new `LiveReader` to WAL pacakge. Added to allow live tailing of a WAL segment, used by dnxware Remote Write after refactor. The main difference between the new reader and the existing `Reader` is that for `LiveReader` a call to `Next()` that returns false does not mean that there will never be more data to read.
